#!/usr/bin/env tsx
/**
 * load-neso-gsp-zones.ts — Load NESO GSP region GeoJSON into network_model.object.
 *
 * Usage:
 *   tsx scripts/load-neso-gsp-zones.ts [--file <path>]
 *
 * Defaults to data/NESO_GSP_regions_4326_20251204.geojson.
...
const CLASS_NAME   = 'GSPZone';
const BATCH_SIZE   = 50;
const DEFAULT_FILE = 'data/NESO_GSP_regions_4326_20251204.geojson';
...
async function main() {
  ...
  // Pre-assign counter-suffixed identities in source order so duplicates always
  // get distinct, stable identities regardless of batch boundaries.
  const gspCounter = new Map<string, number>();
  const identities = features.map(feature => {
    const gspCode = String(feature.properties?.['GSPs'] ?? '');
    const n = gspCounter.get(gspCode) ?? 0;
    gspCounter.set(gspCode, n + 1);
    return `NESO-GSP-${gspCode}#${n}`;
  });
  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    const rows: RowPayload[] = batch.map((feature, batchIdx) => {
      ...
      const gspGroup = String(props['GSPGroup'] ?? '');
      return {
        identity:      identities[i + batchIdx],
        discriminator: gspGroup,
